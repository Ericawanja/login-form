...
const some_data =  (data) => {
  ...
  products_table.innerHTML = data_list.map((item) => {
    return`
    <tr>
    <td>${item.name}</td>
    <td>${item.price}</td>
  ...
  await fetch(
    ...
    .then((data) => {
      ...
      <div class="data_customer_details">
        ...
        <div id='value' class="value">${data.all_time.customers == null ? 0 : data.all_time.customers}</div>
        <div class="title">Customers</div>
    </div>
    <div class="data_transaction_details">
        <span><i class="bi bi-activity"></i></span>
        <div id='value' class="value">${data.all_time.transactions > 1000 ? data.all_time.transactions/1000 +'k': data.all_time.transactions}</div>
        <div class="title">Transactions</div>
    </div>
    <div class="data_sales_details">
        <span><i class="bi bi-receipt"></i></span>
        <div id='value' class="value">${data.all_time.sales > 1000 ? data.all_time.sales/1000 +'k': data.all_time.sales}</div>
